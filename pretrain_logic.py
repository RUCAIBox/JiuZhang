...
from models.data_collator import DataCollatorForLogic
...
def main():
    ...
    def process(examples):
        content = tokenizer(examples['content'], add_special_tokens=False)['input_ids']
        analysis = tokenizer(examples['analysis'], add_special_tokens=False)['input_ids']
        return {'content': content, 'analysis': analysis}
    
    train_dataset = train_dataset.map(process, batched=True)

    data_collator = DataCollatorForLogic(
        tokenizer=tokenizer,
