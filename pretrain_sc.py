...
from models.modeling_cpt import CPTForSC
from models.data_collator import DataCollatorForSC
...
class ModelArguments:
    ...
    fronzen: bool = field(
        default=False,
        metadata={
            "help": "Whether or not fronzen the generator's parameters."
        },
    )
    cross: bool = field(
        default=True,
        metadata={
            "help": "Whether or not cross the adv process."
        },
    )
    decoder_rate: float = field(
        default=0.5,
        metadata={
            "help": "The rate of train decoder."
        },
    )
...
def main():
    ...
    model = CPTForSC.from_pretrained(
        model_args.model_path, 
        config=config, 
        fronzen=model_args.fronzen,
        cross=model_args.cross,
        decoder_rate=model_args.decoder_rate,
    )
    ...
    data_collator = DataCollatorForSC(
        tokenizer=tokenizer,
        use_linear_mask=model_args.use_linear_mask,
